...
export const payrollRecord = sqliteTable('payroll_record', {
  id: integer('id').primaryKey({ autoIncrement: true }),
  payrollSheetId: integer('payroll_sheet_id').notNull().references(() => payrollSheet.id),
  ...
  unq: unique().on(table.payrollSheetId, table.personnelId),
}))
